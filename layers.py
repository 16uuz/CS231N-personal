...
def batchnorm_backward(dout, cache):
    ...
    dmean = np.sum(dxhat * (-1 / std), axis=0) + dvar * np.mean(-2 * (x - mean), axis=0)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    cache = (G, x, x_hat, x_hat_group, mean, var, std, gamma, beta, eps)
    ###########################################################################
    #                             你的代码结束                                #
    ###########################################################################
    return out, cache
...
def spatial_groupnorm_backward(dout, cache):
    ...
    G, x, x_hat, x_hat_group, mean, var, std, gamma, beta, eps = cache
    N, C, H, W = x.shape
    M = (C // G) * H * W
